...
start = text.index('def try_parse_json')
end = text.find('\n\ndef ', start)
if end == -1:
    end = len(text)
new_block = '''def try_parse_json(text: str) -> Optional[dict]:
    """
    ...
    return None
'''
text = text[:start] + new_block + (text[end:] if end < len(text) else '')
parser_path.write_text(text, encoding='utf-8')
print('parser replaced')
